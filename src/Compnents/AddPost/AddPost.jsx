    ...
    return(
        <div>
            <div className='addPostComponent'>
                ...
                <textarea onChange={(e)=>authDispatch({type:'postContent', payload:e.target.value})} value ={authState.postContent} className='textArea' placeholder="What's happening?"></textarea>
                
                <div className='addPostComponentFooter'>
                <span className='photo-icon'><BsImageFill /></span>
                <button onClick={()=>addPost(authState.postContent)} className='addPost-btn'>Post</button>
                </div>
