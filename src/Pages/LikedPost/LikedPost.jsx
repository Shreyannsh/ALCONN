...
export default function LikedPost(){
    ...
    const likedPostByUser = authState?.allPostList?.filter((post) => post.likes.likedBy.find(({_id}) => _id === authState.singleUserDetail._id))
    return(
        <div>
            <h1 className='pageTitle'>Liked Post</h1>
            { likedPostByUser?.map((likedPost) => <li className='list'>
